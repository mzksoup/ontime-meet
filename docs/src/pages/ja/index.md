...
- 使い方は簡単。Google カレンダーの「iCal 形式の限定公開URL」を拡張機能のポップアップに貼り付けるだけで使えます
...
拡張機能に Google カレンダーの iCal 形式の限定公開 URL を設定すると、そのカレンダーから動作に必要な情報（イベントの名前、開始時刻、説明文テキスト、Google Meet の設定有無、リモートミーティングの URL）を定期的に取得し、開始時間の数分前に自動でタブが開くようにタイマーを設定します。取得した情報はその PC 内にのみ保持されており、外部サーバ等へデータの送信・保存は一切行っていません。
...
この Chrome 拡張は、あなたが設定した Google カレンダーの iCal 形式の限定公開 URL から、動作に必要な情報（イベントの名前、開始時刻、説明文テキスト、Google Meet の設定有無、イベントの参加ステータス）を取得します。
